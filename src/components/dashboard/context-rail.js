  ...
  return (
    <section className="card overflow-hidden p-0">
      <div className="flex items-center justify-between gap-3 px-4 py-3">
        ...
        {href ? (
          <ReturnLink href={href} className="text-xs text-muted hover:underline">
            View all
          </ReturnLink>
        ) : null}
      </div>
      <div className="border-t border-base">{children}</div>
    </section>
  );
}
...
function FilterRow({ id, label, value, tone, active, onFocus }) {
  ...
  return (
    ...
    >
      ...
      <span className={cx("text-sm font-semibold tabular-nums", alert && "text-danger")}>
        {value}
      </span>
    </button>
  );
                  ...
                  >
                    ...
                    <Link
                      ...
                      className={cx(
                        ...
                        row.tasks_overdue > 0 && "font-medium text-danger",
                      )}
                    >
            ...
            {activity.slice(0, 6).map((item) => {
              ...
              const body = (
                <div className="px-4 py-2.5">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-xs text-muted">{formatted.title}</div>
                      {formatted.subject ? (
              ...
              return href ? (
                <ReturnLink
                  key={item.id}
                  href={href}
                  className="block hover:bg-accent-soft"
                >
                  {body}
                </ReturnLink>
              ) : (
